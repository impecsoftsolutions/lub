...
| ID | Title | Owner | Scope | Files / Domains | Dependency / Blocker |
|----|-------|-------|-------|-----------------|----------------------|
| CLAUDE-APPEAR-004 | Appearance-System Conformance Audit (passes 1–2 kept; pass 3 reverted) | Claude | Passes 1–2 complete and kept. Pass 3 ("strict linkage") was reverted per user direction — it removed intentional design choices (Footer navy, hero gradients, icon colour diversity). `bg-black/50` overlay consolidation was the only pass-3 change preserved (functionally identical, v4 preferred syntax). `npm run build` PASS; `npm run lint` 0 errors / 3 expected warnings. | All authenticated portal files | Complete (with pass-3 revert). |
| COD-APPEAR-003 | Appearance hardcode audit + safe batch replacements | Codex | Audit authenticated admin/member surfaces for hardcoded design values and replace the low-risk background/border/text/table-wrapper violations with theme-aware tokens | `src/pages/Admin*.tsx`, `src/pages/admin/*.tsx`, `src/pages/Member*.tsx`, `src/components/admin/**`, `src/components/dashboard/**`, `src/components/*.tsx` | Complete locally. `npm run build` PASS; `npm run lint` 0 errors / 3 expected warnings. |
| CLAUDE-APPEAR-002 | Appearance customization expansion + full typography audit | Claude | Font Family (7 fonts + Google Fonts preload), Typography (5 roles × size+weight), Corner Style (numeric slider/input replacing 3-preset buttons), Table Style (Row Spacing + Scrollbar Thickness), table shadow fix for native tables, native table cell padding CSS tokens, 34 remaining `<th>` elements fixed across 4 admin pages | `src/pages/admin/AdminAppearanceSettings.tsx`, `src/lib/themeManager.ts`, `src/index.css`, `src/pages/AdminCityManagement.tsx`, `src/pages/AdminDesignationsManagement.tsx`, `src/pages/AdminStateManagement.tsx`, `src/pages/admin/AdminUsers.tsx` | Complete. `npm run build` PASS; `npm run lint` 0 errors / 3 expected warnings. |
| COD-JOIN-001 | Fix Join verification/submission loading-state regression | Codex | Correct the crossed loading-state flags so Verify drives `isVerifying` and actual registration submit drives `isSubmitting`, restoring proper button text/disabled behavior around normalization | `src/pages/Join.tsx`, Join registration flow | Complete locally. `npm run build` PASS; `npm run lint` 0 errors / 3 expected warnings. |
| CLAUDE-TYPE-001 | Unified admin/dashboard typography scale | Claude | Phase 1: Added `text-section` (15px) and `text-label` (13px) tokens + fixed 5 shared shadcn components. Phase 2: Fixed all individual admin/dashboard page files. Phase 3: Fixed all component files. Final grep confirms zero remaining violations in admin/dashboard scope. | `src/index.css`, `src/components/ui/dialog.tsx`, `src/components/ui/card.tsx`, `src/components/ui/label.tsx`, `src/components/ui/table.tsx`, `src/components/ui/PageHeader.tsx`, 20+ page files, 14 component files | Complete. Build PASS, lint 0 errors. |
