/*
  Business Showcase contact cleanup:
  - Add optional public website URL.
  - Validate optional contact email and website URL server-side.
  - Keep contact phone free-form because it may be mobile, landline, or formatted text.
  - Treat non-empty email/phone/website as public by design.
*/
...
CREATE OR REPLACE FUNCTION public.is_valid_showcase_email(p_email text)
RETURNS boolean
LANGUAGE sql
IMMUTABLE
AS $$
  SELECT COALESCE(trim(p_email), '') ~* '^[A-Z0-9._%+\-]+@[A-Z0-9.\-]+\.[A-Z]{2,}$';
$$;

CREATE OR REPLACE FUNCTION public.normalize_showcase_website_url(p_url text)
RETURNS text
LANGUAGE plpgsql
IMMUTABLE
AS $$
...
BEGIN
  ...
  IF v_url !~* '^https?://([a-z0-9]([a-z0-9-]{0,61}[a-z0-9])?\.)+[a-z]{2,}(:[0-9]{1,5})?(/[^\s]*)?$' THEN
    RETURN NULL;
  END IF;
...
DROP FUNCTION IF EXISTS public.create_showcase_listing_with_session(text, text, text, text, text, text, jsonb, text, text, boolean, boolean);
...
GRANT EXECUTE ON FUNCTION public.create_showcase_listing_with_session(text, text, text, text, text, text, jsonb, text, text, boolean, boolean, text) TO anon, authenticated;

DROP FUNCTION IF EXISTS public.update_showcase_listing_with_session(text, uuid, text, text, text, text, text, jsonb, text, text, boolean, boolean);
...
GRANT EXECUTE ON FUNCTION public.update_showcase_listing_with_session(text, uuid, text, text, text, text, text, jsonb, text, text, boolean, boolean, text) TO anon, authenticated;
