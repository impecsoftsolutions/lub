/*
  COD-DESIGNATIONS-ALTERNATE-CONTACT-LEADERSHIP-MOBILE-PHOTO-093

  Adds alternate-contact mobile and optional photo URL to role assignments so
  the leadership page can display correct contact details for alternate rows
  without ever falling back to the main member's mobile/photo.

  Schema changes
  ──────────────
  member_lub_role_assignments:
  • alternate_contact_mobile_snapshot  text (nullable)
  • alternate_contact_photo_url_snapshot text (nullable)

  Backfill: existing alternate rows receive mobile snapshot from
  member_registrations.alternate_mobile (photo stays NULL).

  RPC changes  (all via DROP + CREATE because RETURNS TABLE columns change)
  ──────────────────────────────────────────────────────────────────────────
  • admin_assign_member_lub_role           — add p_alternate_mobile, p_alternate_photo_url (13 params)
  • admin_assign_member_lub_role_with_session — pass-through for new params (13 params)
  • admin_get_member_lub_role_assignments  — add both snapshots to RETURNS TABLE (25 cols)
  • admin_get_member_lub_role_assignments_with_session — same
  • get_public_leadership_assignments      — add both snapshots to RETURNS TABLE (22 cols)
...
ALTER TABLE public.member_lub_role_assignments
  ADD COLUMN IF NOT EXISTS alternate_contact_mobile_snapshot   text,
  ADD COLUMN IF NOT EXISTS alternate_contact_photo_url_snapshot text;

-- ═══════════════════════════════════════════════════════════════════════════
-- 2. Backfill existing alternate rows: populate mobile from member_registrations
--    Photo stays NULL (no source column on member_registrations).
-- ═══════════════════════════════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════════════════════════════
-- 3. Recreate admin_assign_member_lub_role (11 → 13 params)
-- ═══════════════════════════════════════════════════════════════════════════

DROP FUNCTION IF EXISTS public.admin_assign_member_lub_role(uuid, uuid, uuid, text, text, text, date, date, text, text, text);
...
-- ═══════════════════════════════════════════════════════════════════════════
-- 4. Recreate admin_assign_member_lub_role_with_session (11 → 13 params)
-- ═══════════════════════════════════════════════════════════════════════════

DROP FUNCTION IF EXISTS public.admin_assign_member_lub_role_with_session(text, uuid, uuid, text, text, text, date, date, text, text, text);
...
-- ═══════════════════════════════════════════════════════════════════════════
-- 5. Recreate admin_get_member_lub_role_assignments (23 → 25 cols)
-- ═══════════════════════════════════════════════════════════════════════════
...
RETURNS TABLE (
  ...
  alternate_contact_mobile_snapshot    text,
  alternate_contact_photo_url_snapshot text
)
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
  ...
  SELECT
    ...
    a.alternate_contact_mobile_snapshot    AS alternate_contact_mobile_snapshot,
    a.alternate_contact_photo_url_snapshot AS alternate_contact_photo_url_snapshot
  FROM member_lub_role_assignments a
  ...
  WHERE
    ...
    AND (
      ...
      OR COALESCE(a.alternate_contact_mobile_snapshot, '') ILIKE '%' || p_search || '%'
    )
  ORDER BY
    r.display_order ASC,
    r.role_name     ASC,
...
-- ═══════════════════════════════════════════════════════════════════════════
-- 6. Recreate admin_get_member_lub_role_assignments_with_session (25 cols)
-- ═══════════════════════════════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════════════════════════════
-- 7. Recreate get_public_leadership_assignments (20 → 22 cols)
-- ═══════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  SELECT
    ...
    a.alternate_contact_mobile_snapshot,
    a.alternate_contact_photo_url_snapshot
  FROM member_lub_role_assignments a
