/*
  # Add approved members export RPC for admin XLSX download

  1. Purpose
    - Provide a narrow, session-wrapped backend read contract for approved member export
    - Keep export separate from the broader admin registrations list path
    - Reuse the existing members permission model without introducing a new public query path
*/
...
COMMENT ON FUNCTION public.get_admin_approved_members_export_with_session(text) IS
  'Session-wrapped admin export contract for approved members list download (company, member, city, district).';
