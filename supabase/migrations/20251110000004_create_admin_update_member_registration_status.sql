/*
  # Create admin_update_member_registration_status - Workaround for PostgREST Caching Issue

  ## Problem
  - Persistent "cannot pass more than 100 arguments to a function" error
  - PostgREST appears to have cached metadata issues with 'admin_update_registration_status'
  - Function definition is correct but PostgREST routing fails

  ## Solution
  - Create new function with different name: admin_update_member_registration_status
  - Use exact same logic from working migration 20251110000003
  - Update client code to call new function name
  - Bypasses any PostgREST caching/routing issues

  ## Implementation
  1. Create new function with name: admin_update_member_registration_status
  2. Copy exact logic from admin_update_registration_status
  3. Grant appropriate permissions
  4. Keep old function for backward compatibility (if needed later)
*/
...
COMMENT ON FUNCTION admin_update_member_registration_status(uuid, uuid, text, text) IS
  'SECURITY DEFINER RPC to update member registration status (approve/reject). This is a workaround for PostgREST caching issues with admin_update_registration_status. Validates user permissions, updates status, updates user account_type when approving, logs audit trail. JavaScript client ALWAYS passes all 4 parameters.';
...
DO $$
BEGIN
  RAISE NOTICE '✓ Created admin_update_member_registration_status function';
  RAISE NOTICE '✓ Function signature: admin_update_member_registration_status(uuid, uuid, text, text DEFAULT NULL)';
  RAISE NOTICE '✓ Workaround for PostgREST caching issue with admin_update_registration_status';
  RAISE NOTICE '✓ Client code must be updated to call new function name';
END $$;
