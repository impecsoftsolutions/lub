/*
  # Fix get_public_leadership_assignments - Correct Column Names

  1. Issue
    - get_public_leadership_assignments references member_registrations.is_deleted
    - This column does NOT exist in the schema
    - Also potentially referencing incorrect column names

  2. Actual Schema (from member_registrations)
    - full_name (text)
    ...
    - profile_photo_url (text, added separately)

  3. Fix
    - Use correct column names from member_registrations
    - Filter for is_active = true AND status = 'approved'
    - Return data with same output column names TS expects
*/

-- =====================================================================
-- Fix get_public_leadership_assignments: Use correct column names
-- =====================================================================
...
BEGIN
  ...
  WHERE 
    ...
    -- Only show active roles and active/approved members
...
COMMENT ON FUNCTION public.get_public_leadership_assignments(text, text, text, date, text) IS
  'Public read-only function for Leadership page. Returns leadership assignments filtered by level, region, committee year, and active date range. Fixed to use correct column names (is_active, status) from member_registrations. Includes member gender for Shri./Smt. prefix, company info, city, district, and profile photo. Safe for anon access with only display-appropriate fields.';
