/*
  # Create update_member_registration_status - Another Workaround for PostgREST Caching

  ## Problem
  - Both admin_update_registration_status and admin_update_member_registration_status fail
  - PostgREST caching issues persist with 'admin_' prefixed function names
  - Error: "cannot pass more than 100 arguments to a function"

  ## Solution
  - Create function with simpler name: update_member_registration_status (no 'admin_' prefix)
  - Use exact same logic that works via SQL
  - Update client to call new function name
  - Bypass PostgREST caching completely with new name

  ## Implementation
  1. Create new function: update_member_registration_status
  2. Copy exact working logic
  3. Grant permissions
  4. Client will call this new function
*/

-- ============================================================================
-- CREATE NEW FUNCTION WITH SIMPLER NAME
-- ============================================================================
...
COMMENT ON FUNCTION update_member_registration_status(uuid, uuid, text, text) IS
  'SECURITY DEFINER RPC to update member registration status (approve/reject). Workaround for PostgREST caching with simpler function name (no admin_ prefix). Validates permissions, updates status, updates account_type, logs audit trail. Always pass all 4 parameters.';
...
DO $$
BEGIN
  RAISE NOTICE '✓ Created update_member_registration_status function';
  RAISE NOTICE '✓ Function signature: update_member_registration_status(uuid, uuid, text, text DEFAULT NULL)';
  RAISE NOTICE '✓ Simpler name without admin_ prefix to bypass PostgREST caching';
  RAISE NOTICE '✓ Client code must be updated to call this function';
END $$;
