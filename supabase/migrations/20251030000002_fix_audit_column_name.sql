/*
  # Fix Member Audit History Column Name

  ## Problem
  The RPC function update_member_registration references a column "changed_at"
  in the member_audit_history table, but the actual column name is "created_at".

  ## Root Cause
  Line 261 in migration 20251030000001_fix_member_id_duplicate_constraint.sql:
  ```sql
  INSERT INTO member_audit_history (
    ...
    new_value,
    changed_by,
    changed_at  -- WRONG: Column doesn't exist
  )
  ```

  ## Actual Schema
  From migration 20251004044045_create_member_audit_history_table.sql:
  - Column is named "created_at" (timestamptz DEFAULT now())
  - Not "changed_at"

  ## Solution
  Update the INSERT statement to use the correct column name "created_at".
*/

-- Drop existing function
DROP FUNCTION IF EXISTS update_member_registration(uuid, uuid, jsonb, boolean);

-- Create corrected version with fixed audit history column name
...
BEGIN
  ...
  LOOP
    ...
    IF v_old_value IS DISTINCT FROM v_new_value THEN
      -- FIXED: Use correct column name "created_at" instead of "changed_at"
      INSERT INTO member_audit_history (
        member_id,
        action_type,
        field_name,
...
COMMENT ON FUNCTION update_member_registration(uuid, uuid, jsonb, boolean) IS
  'SECURITY DEFINER RPC to update member registrations. FIXED: Audit logging uses correct column name "created_at". Member ID only updates when value changes.';
...
BEGIN
  ...
  RAISE NOTICE '✓ Audit logging now uses correct column: "created_at"';
  RAISE NOTICE '✓ Previously incorrectly referenced: "changed_at"';
  RAISE NOTICE '✓ All functionality preserved from previous migration';
END $$;
