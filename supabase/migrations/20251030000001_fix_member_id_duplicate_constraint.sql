/*
  # Fix Member ID Duplicate Constraint Error

  ## Problem
  When editing a member with an existing member_id, the RPC function attempts to
  update the member_id field even when the value hasn't changed. This triggers
  PostgreSQL's unique constraint check, resulting in:
  "duplicate key value violates unique constraint 'member_registrations_member_id_key'"

  ## Root Cause
  Line 167 in the previous RPC function:
  ```sql
  member_id = COALESCE((v_update_data->>'member_id'), member_id),
  ```

  This always attempts to set member_id, even when the new value equals the current
  value. PostgreSQL's unique constraint check runs before recognizing it's the same row.

  ## Solution
  Update the member_id assignment logic to:
  - Only update member_id if a new value is provided
  - AND the new value is not empty
  - AND the new value differs from the current value
  - Otherwise, keep the existing member_id unchanged

  This prevents unnecessary updates and avoids triggering the unique constraint when
  the value isn't actually changing.

  ## Changes
  Modified member_id update logic from COALESCE to CASE statement with proper conditions.
*/

-- Drop existing function
DROP FUNCTION IF EXISTS update_member_registration(uuid, uuid, jsonb, boolean);

-- Create corrected version with fixed member_id logic
...
BEGIN
  ...
  SET
    ...
    -- FIXED: Only update member_id if value is actually changing
    member_id = CASE
...
COMMENT ON FUNCTION update_member_registration(uuid, uuid, jsonb, boolean) IS
  'SECURITY DEFINER RPC to update member registrations for custom auth system. FIXED: member_id only updates when value actually changes, preventing duplicate constraint violations.';
...
BEGIN
  ...
  RAISE NOTICE '✓ member_id now only updates when value actually changes';
  RAISE NOTICE '✓ Empty string member_id values converted to NULL';
  RAISE NOTICE '✓ Prevents duplicate constraint violations on unchanged values';
  RAISE NOTICE '✓ Non-super-admins can no longer modify member_id field';
END $$;
