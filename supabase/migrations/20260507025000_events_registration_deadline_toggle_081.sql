/*
  COD-EVENTS-REGISTRATION-DEADLINE-TOGGLE-081

  Adds explicit deadline-toggle behavior for Event registration:
  - `ai_metadata.rsvp_deadline_enabled = true`:
      - enforce custom `rsvp_deadline_at` (fallback to event end/start if missing)
      - show deadline on public website
  - `ai_metadata.rsvp_deadline_enabled = false` (or absent with no custom deadline):
      - hide deadline on public website
      - enforce implicit deadline at event end time (fallback start time)
*/
...
BEGIN
  ...
  IF v_email IS NOT NULL THEN
    ...
    LIMIT 1;
    IF FOUND THEN
      SELECT b.badge_code INTO v_existing_badge_code
      ...
      RETURN jsonb_build_object(
        ...
        'badge_code', v_existing_badge_code
      );
    END IF;
  END IF;

  IF v_phone_digits IS NOT NULL THEN
    SELECT * INTO v_existing FROM public.event_rsvps r
    WHERE r.event_id = v_event.id
      AND regexp_replace(COALESCE(r.phone, ''), '\D', '', 'g') = v_phone_digits
    ...
    IF FOUND THEN
      ...
      RETURN jsonb_build_object(
        ...
        'error', 'A registration already exists with this mobile number',
        'duplicate', true,
  ...
  IF v_aadhaar IS NOT NULL THEN
    SELECT * INTO v_existing FROM public.event_rsvps r
    WHERE r.event_id = v_event.id
      AND r.aadhaar_number = v_aadhaar
      AND r.status IN ('confirmed','pending','waitlisted')
    LIMIT 1;
    IF FOUND THEN
      SELECT b.badge_code INTO v_existing_badge_code
      ...
      RETURN jsonb_build_object(
        'success', false,
        'error_code', 'duplicate_aadhaar',
        'error', 'A registration already exists with this Aadhaar Card number',
        'duplicate', true,
        'duplicate_field', 'aadhaar',
        'rsvp_id', v_existing.id,
        'badge_code', v_existing_badge_code
      );
    END IF;
