/*
  Business Showcase keywords:
  - Add optional keywords text for search.
  - Return keywords through public/member/admin read RPCs.
  - Accept keywords in create/update RPCs.
  - Include keywords in public/admin search matching.
*/
...
CREATE OR REPLACE FUNCTION public.normalize_showcase_keywords(p_keywords text)
RETURNS text
LANGUAGE sql
IMMUTABLE
AS $$
  SELECT nullif(left(regexp_replace(trim(COALESCE(p_keywords, '')), '\s+', ' ', 'g'), 500), '');
$$;
...
DROP FUNCTION IF EXISTS public.create_showcase_listing_with_session(text, text, text, text, text, text, jsonb, text, text, boolean, boolean, text);
...
BEGIN
  ...
  INSERT INTO public.showcase_listings (
    ...
    title, product_service_name, category, keywords,
...
GRANT EXECUTE ON FUNCTION public.create_showcase_listing_with_session(text, text, text, text, text, text, jsonb, text, text, boolean, boolean, text, text) TO anon, authenticated;

DROP FUNCTION IF EXISTS public.update_showcase_listing_with_session(text, uuid, text, text, text, text, text, jsonb, text, text, boolean, boolean, text);
