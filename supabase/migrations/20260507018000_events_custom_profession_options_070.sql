-- COD-EVENTS-PROFESSION-OPTIONS-070
-- Make Event registration Profession dropdown options configurable per event.


-- COD-EVENTS-PROFESSION-OPTIONS-070
-- Profession options are now configurable per event through events.ai_metadata.
-- Existing events keep the legacy default values below.
ALTER TABLE public.event_rsvps
  DROP CONSTRAINT IF EXISTS event_rsvps_profession_check;

CREATE OR REPLACE FUNCTION public.event_rsvp_profession_options(p_meta jsonb)
RETURNS jsonb
LANGUAGE plpgsql
IMMUTABLE
...
AS $$
DECLARE
  v_raw jsonb := COALESCE(p_meta->'rsvp_profession_options', '[]'::jsonb);
...
BEGIN
  IF jsonb_typeof(v_raw) = 'array' THEN
    FOR v_item IN SELECT value FROM jsonb_array_elements(v_raw)
    LOOP
      IF jsonb_typeof(v_item) = 'string' THEN
        v_label := trim(both '"' from v_item::text);
      ELSIF jsonb_typeof(v_item) = 'object' THEN
        v_label := COALESCE(NULLIF(trim(v_item->>'label'), ''), NULLIF(trim(v_item->>'value'), ''));
      ELSE
        v_label := NULL;
      END IF;
      v_label := NULLIF(trim(regexp_replace(COALESCE(v_label, ''), '\s+', ' ', 'g')), '');
      ...
      v_result := v_result || jsonb_build_array(jsonb_build_object('value', v_label, 'label', v_label));
      IF jsonb_array_length(v_result) >= 20 THEN
        EXIT;
      END IF;
    END LOOP;
  END IF;
  ...
  RETURN jsonb_build_array(
    jsonb_build_object('value', 'company_owner', 'label', 'Company Owner'),
    jsonb_build_object('value', 'director', 'label', 'Director'),
    jsonb_build_object('value', 'official', 'label', 'Official'),
...
GRANT EXECUTE ON FUNCTION public.event_rsvp_profession_options(jsonb) TO anon, authenticated;
