/*
  # Fix admin_update_registration_status - Single Function with DEFAULT NULL

  ## Problem
  - PostgREST/Supabase does not properly support function overloading
  - JavaScript conditionally includes/excludes p_rejection_reason parameter
  - PostgREST cannot route RPC calls to correct overload
  - Error: "cannot pass more than 100 arguments to a function"
  - This error occurs when PostgREST can't find matching function signature

  ## Root Cause
  - Function overloading by parameter count doesn't work with PostgREST's RPC mechanism
  - PostgREST treats JavaScript object keys as named parameters
  - Conditional parameter inclusion creates ambiguity

  ## Solution (Option 1)
  - Create SINGLE function with DEFAULT NULL for p_rejection_reason
  - JavaScript will ALWAYS pass all 4 parameters (including null when not needed)
  - Eliminates ambiguity for PostgREST
  - Maintains atomic operation in single RPC call

  ## Implementation
  1. Drop both existing function overloads
  2. Create single function with signature:
     admin_update_registration_status(uuid, uuid, text, text DEFAULT NULL)
  3. Keep all business logic from existing function
  4. Grant appropriate permissions
*/

-- ============================================================================
-- STEP 1: DROP EXISTING FUNCTION OVERLOADS
-- ============================================================================

-- Drop all existing function signatures completely
...
-- ============================================================================
-- STEP 2: CREATE SINGLE FUNCTION WITH DEFAULT NULL
-- ============================================================================
...
COMMENT ON FUNCTION admin_update_registration_status(uuid, uuid, text, text) IS
  'SECURITY DEFINER RPC to update member registration status (approve/reject) for custom auth system. Single function with DEFAULT NULL for p_rejection_reason eliminates PostgREST function overload ambiguity. Validates user permissions, updates status, updates user account_type when approving, logs audit trail. JavaScript client ALWAYS passes all 4 parameters.';
...
DO $$
BEGIN
  RAISE NOTICE '✓ Fixed admin_update_registration_status with single function';
  RAISE NOTICE '✓ Function signature: admin_update_registration_status(uuid, uuid, text, text DEFAULT NULL)';
  RAISE NOTICE '✓ Eliminated PostgREST function overload ambiguity';
  RAISE NOTICE '✓ JavaScript client must ALWAYS pass all 4 parameters (including null)';
  RAISE NOTICE '✓ Handles approvals and rejections in single atomic operation';
END $$;
