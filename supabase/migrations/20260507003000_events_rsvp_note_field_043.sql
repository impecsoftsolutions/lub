/*
  # COD-EVENTS-RSVP-FIELD-LIST-FIX-043
  - Surface and enforce per-event Note field controls.
  - Reuses the existing ai_metadata pattern (042) so no schema columns added.
  - Public detail RPC returns rsvp.collect_note + rsvp.require_note.
  - Submit RPC enforces note_required when collected + required.
*/

-- =============================================================================
-- get_event_by_slug — surface collect_note + require_note
-- =============================================================================
...
-- =============================================================================
-- submit_event_rsvp — enforce note_required when collected + required
-- (re-creates the 042 function body and adds the note check just before
--  the existing required-field checks return.)
-- =============================================================================
...
BEGIN
  ...
  END IF;
  -- 043: Note required when collected + required.
  ...
  END IF;
  -- If Note collection is OFF, ignore any submitted note silently to keep the
  -- public form contract consistent with admin configuration.
  ...
  IF v_event.capacity_mode = 'per_day' AND v_event.per_day_capacity IS NOT NULL THEN
    IF v_visit_all_days THEN
      -- All-day attendees consume capacity on every event day; reject if any day is full.
      FOR v_day IN
